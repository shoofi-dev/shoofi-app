...
const OrderItems = ({ order, menu }) => {
  const { t } = useTranslation();
  const { menuStore, languageStore } = useContext(StoreContext);

  // Function to find meal data from menu
  const findMealFromMenu = (itemId, menu) => {
    if (!menu) return null;
    console.log("XXXXX")
  ...
  const renderOrderItems = (order) => {
    return order.order.items?.map((item, index) => {
      ...
      // Find the meal data from menu
      console.log("item._id", item.item_id);
      const meal = findMealFromMenu(item.item_id, menu);
      console.log("Found meal:", meal);
      // if (meal && meal.extras) {
      //   console.log("Meal extras structure:", JSON.stringify(meal.extras));
      //   console.log("Selected extras:", JSON.stringify(item.selectedExtras));
      // }
        ...
        <View style={{}}>
          ...
          >
            ...
            >
              ...
              {/* Render extras using OrderExtrasDisplay */}
              <OrderExtrasDisplay
